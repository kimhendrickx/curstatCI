...
List ComputeConfIntervals(DataFrame input, NumericVector x, double alpha)
{
    ...
    for (i=1;i<=npoints;i++)
    {
        ...
        // Rprintf("%16.10f %16.10f %16.10f\n", grid[10*i], hmin1[i], sqrt(varF(N,n,frequence1,y,data0[1],data0[n],data0,hmin1[i],grid[10*i])) );
    }
    ...
    //ofstream file_("/Users/pietgroeneboom/Documents/curstat_package/CI_SMLE.txt");
